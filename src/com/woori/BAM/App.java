        ...
        while (true) {
            ...
            if (cmd.equals("article list")) {
                ...
                for (int i = articles.size() - 1; i >= 0; i--) {
                    ...
                    System.out.printf("%d    |    %s   |  %s  |   %s  |  %d\n", article.getId(), article.getTitle(), article.getBody(), article.getRegDate(), article.getViewCnt());  //static 메서드 호출
                }
                ...
                for (Article article : articles) {
                    if (article.getId() ==id){      // read -> getter
...
//                articles.remove(id-1);        //remove(int index) 1번 방식
    ...
    void makeTestData() {  // static 메서드안에서는 static 멤버 필드를 사용한다, 일반 멤버 필드는 사용불가
